...
const AdminSideBar = () => {
    let p  =  [1]
    const activeLink = 'w-full px-4 py-2 my-2 sm:text-xl rounded-xl flex items-center gap-2 lg:gap-4 bg-gray-200 text-cyan-700 backdrop-hue-rotate-0'
  ...
  return (
    <div className='hidden sm:flex flex-col items-center px-4 sm:px-6  w-44 md:w-56 lg:w-72 border-r md:overflow-hidden overflow-auto md:hover:overflow-auto my-5'>
        {p.map(pp => (
        <NavLink to='posts' onClick={() => {}}  className={({ isActive}) => isActive ? activeLink : normalLink}>
                 <MdOutlineArticle/>
